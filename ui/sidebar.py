...
def render_model_inputs(defaults: ModelInputs | None = None, prefix: str = "mi",
    ...
    with st.sidebar.expander("Starting State", expanded=False):
        ...
        inp.assets = st.number_input("Assets ($)", value=inp.assets, step=1000.0, key=f"{prefix}_assets")
        inp.liabilities = st.number_input("Liabilities ($)", value=inp.liabilities, step=1000.0, key=f"{prefix}_liabilities")
        inp.customer_count = int(st.number_input(
        ...
        inp.debt = st.number_input("Debt ($)", value=inp.debt, step=1000.0, key=f"{prefix}_debt")
        inp.interest_rate = st.number_input(
    ...
    with st.sidebar.expander("Marketing Channels", expanded=True):
        ...
        if inp.use_inbound:
            ...
            inp.funnel_conversion_rate = st.number_input("Funnel Conv (% click→lead)", value=inp.funnel_conversion_rate, step=0.5, key=f"{prefix}_fcr")
            inp.time_to_market_inbound = int(st.number_input("Time to Market (days)", value=inp.time_to_market_inbound, step=1, key=f"{prefix}_ttm_in"))
            inp.lead_conversion_rate_inbound = st.number_input("Lead→Customer (%)", value=inp.lead_conversion_rate_inbound, step=1.0, key=f"{prefix}_lcr_in")
        ...
        if inp.use_outbound:
            ...
            inp.outbound_conversion_rate = st.number_input("Contact→Lead (%)", value=inp.outbound_conversion_rate, step=0.5, key=f"{prefix}_ocr")
            inp.time_to_market_outbound = int(st.number_input("Time to Market (days)", value=inp.time_to_market_outbound, step=1, key=f"{prefix}_ttm_ob"))
            inp.lead_conversion_rate_outbound = st.number_input("Lead→Customer (%)", value=inp.lead_conversion_rate_outbound, step=1.0, key=f"{prefix}_lcr_ob")
        ...
        if inp.use_organic:
            ...
            inp.time_to_market_organic = int(st.number_input("Time to Market (days)", value=inp.time_to_market_organic, step=1, key=f"{prefix}_ttm_org"))
            inp.organic_cost_per_month = st.number_input("Organic Cost ($/month)", value=inp.organic_cost_per_month, step=500.0, key=f"{prefix}_org_cost")

    # ── Product ────────────────────────────────────────────────────
    with st.sidebar.expander("Product", expanded=False):
        inp.price_of_offer = st.number_input("Price of Offer — P ($)", value=inp.price_of_offer, step=100.0, key=f"{prefix}_price")
        inp.realization_rate = st.number_input("Realization Rate (%)", value=inp.realization_rate, step=1.0, key=f"{prefix}_rr")
        inp.cost_to_fulfill = st.number_input("Cost to Fulfill (% of P)", value=inp.cost_to_fulfill, step=1.0, key=f"{prefix}_cf")
        inp.time_to_collect = int(st.number_input("Time to Collect (days)", value=inp.time_to_collect, step=30, key=f"{prefix}_ttc"))
        inp.refund_period = int(st.number_input("Refund Period (days)", value=inp.refund_period, step=10, key=f"{prefix}_refp"))
        inp.refund_rate = st.number_input("Refund Rate (%)", value=inp.refund_rate, step=0.5, key=f"{prefix}_refr")
        inp.contract_length = int(st.number_input("Contract Length (days)", value=inp.contract_length, step=30, key=f"{prefix}_cl"))

    # ── Renewals ───────────────────────────────────────────────────
    with st.sidebar.expander("Renewals", expanded=False):
        inp.churn_rate = st.number_input("Churn Rate (%)", value=inp.churn_rate, step=5.0, key=f"{prefix}_churn")
        if inp.churn_rate < 100:
            inp.price_of_renewal = st.number_input("Price of Renewal ($)", value=inp.price_of_renewal, step=500.0, key=f"{prefix}_pren")
            inp.cost_to_sell_renewal = st.number_input("Cost to Sell Renewal (%)", value=inp.cost_to_sell_renewal, step=1.0, key=f"{prefix}_cs_ren")
            inp.cost_to_fulfill_renewal = st.number_input("Cost to Fulfill Renewal (%)", value=inp.cost_to_fulfill_renewal, step=1.0, key=f"{prefix}_cf_ren")
            inp.time_to_collect_renewal = int(st.number_input("Time to Collect Renewal (days)", value=inp.time_to_collect_renewal, step=10, key=f"{prefix}_ttc_ren"))
            inp.renewal_rate_of_renewals = st.number_input("Renewal of Renewals (%)", value=inp.renewal_rate_of_renewals, step=5.0, key=f"{prefix}_ror")

    # ── Viral Component ────────────────────────────────────────────
    with st.sidebar.expander("Viral Component", expanded=False):
        inp.use_viral = st.checkbox("Enable Viral", value=inp.use_viral, key=f"{prefix}_use_viral")
        if inp.use_viral:
            inp.invites_per_customer = st.number_input("Invites per Customer", value=inp.invites_per_customer, step=1.0, key=f"{prefix}_invites")
            inp.conversion_rate_per_invite = st.number_input("Invite Conv (%)", value=inp.conversion_rate_per_invite, step=1.0, key=f"{prefix}_viral_conv")
            inp.viral_time = int(st.number_input("Viral Time (days)", value=inp.viral_time, step=5, key=f"{prefix}_viral_time"))
            inp.viral_start = int(st.number_input("Viral Start (day)", value=inp.viral_start, step=30, key=f"{prefix}_viral_start"))
            inp.cost_to_sell_viral = st.number_input("Cost to Sell Viral (%)", value=inp.cost_to_sell_viral, step=1.0, key=f"{prefix}_cs_viral")
            inp.cost_to_market_viral = st.number_input("Cost to Market Viral (%)", value=inp.cost_to_market_viral, step=1.0, key=f"{prefix}_cm_viral")

    # ── Administration ─────────────────────────────────────────────
    with st.sidebar.expander("Administration", expanded=False):
        inp.transaction_fee = st.number_input("Transaction Fee (%)", value=inp.transaction_fee, step=0.1, key=f"{prefix}_tf")
        inp.fixed_costs_per_month = st.number_input("Fixed Costs ($/month)", value=inp.fixed_costs_per_month, step=1000.0, key=f"{prefix}_fc")
        inp.fixed_cost_increase_per_100_customers = st.number_input("FC Increase per 100 Customers ($/month)", value=inp.fixed_cost_increase_per_100_customers, step=500.0, key=f"{prefix}_fc_scale")

    # ── Valuation ──────────────────────────────────────────────────
    with st.sidebar.expander("Valuation Parameters", expanded=False):
        inp.tax_rate = st.number_input("Tax Rate (%)", value=inp.tax_rate, step=1.0, key=f"{prefix}_tax")
        inp.discount_rate = st.number_input("Discount Rate (%)", value=inp.discount_rate, step=0.5, key=f"{prefix}_dr")
        inp.perpetual_growth_rate = st.number_input("Perpetual Growth (%)", value=inp.perpetual_growth_rate, step=0.5, key=f"{prefix}_pgr")
        inp.time_max = int(st.number_input("Simulation Period (days)", value=inp.time_max, step=100, key=f"{prefix}_tmax"))
        inp.number_of_shares = int(st.number_input("Number of Shares", value=inp.number_of_shares, step=100_000, key=f"{prefix}_shares"))
        inp.enterprise_multiple_ebitda = st.number_input("EBITDA Multiple", value=inp.enterprise_multiple_ebitda, step=1.0, key=f"{prefix}_ebitda_mult")
